...
pub fn cli_match_msg(cli: &Cli) -> Result<()> {
    if let Some(Mode::Msg { src_file, dec, key_file, any_file, passwd, base256mode_code}) = &cli.mode{
        ...
        match (src_file, dec, key_file, any_file, passwd) {
            // 随机密钥模式
            ...
            (Some(src_file), false, None, None, false) => {
            ...
            (Some(src_file), false, Some(key_file), None, false) => {
                let pt_msg: Zeroizing<String> = msg_read_file(src_file)?;
                let key: Zeroizing<[u8; 32]> = load_key(key_file,None)?;
                msg_encrypt(pt_msg, &key, false, base256mode_code)?;
            }
            ...
            (Some(src_file), false, None, Some(any_file), &use_passwd) => {
                let pt_msg: Zeroizing<String> = msg_read_file(src_file)?;
                let need_confirm = use_passwd; // 加密时需要确认密码
                let key: Zeroizing<[u8; 32]> = derive_key_from_any_file(any_file, use_passwd, need_confirm)?;
                msg_encrypt(pt_msg, &key, false, base256mode_code)?;
            ...
            (Some(src_file), false, None, None, true) => {
                let pt_msg: Zeroizing<String> = msg_read_file(src_file)?;
                let passwd: Zeroizing<String> = read_passwd_interactive()?;
                let key: Zeroizing<[u8; 32]> = derive_key_from_password(passwd)?;
                msg_encrypt(pt_msg, &key, false, base256mode_code)?;
...
pub fn cli_match_base(cli: &Cli) -> Result<()> {
    if let Some(Mode::Base { io_encode, io_decode, src_file, dec_file, base256mode_code }) = &cli.mode{
        my_println!("您正在使用编码模式，该模式不是加密，不提供任何安全性，不应用于任何隐私内容");
        let base256mode_code = *base256mode_code;
        match (io_encode, io_decode, src_file, dec_file) {
            (true, false, None, None) => {
                base_encode_io(base256mode_code)?;
            }
            (false, true, None, None) => {
                base_decode_io(base256mode_code)?;
            }

            (false, false, Some(src_file), None) => {
                base_encode(src_file, base256mode_code)?;
            }
            (false, false, None, Some(dec_file)) => {
