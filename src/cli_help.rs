...
使用示例:
  ...
  kitty_enc [-k KEY_FILE] -s DIR    # [使用指定密钥]加密指定目录（优先在 DIR 中查找密钥，没有则生成，默认使用密码短语)
  kitty_enc [-k KEY_FILE] -d DIR    # [使用指定密钥]解密指定目录（在 DIR 中查找密钥文件，默认使用密码短语）
  kitty_enc -p                      # 纯密码模式：使用密码加密当前目录（不生成密钥文件）
  kitty_enc -p -s DIR               # 纯密码模式：使用密码加密指定目录（不生成密钥文件）
  kitty_enc -p -d DIR               # 纯密码模式：使用密码解密指定目录（不生成密钥文件）
  kitty_enc -a FILE [-p]            # 任意文件模式[+密码]：使用文件内容[和密码]派生密钥加密当前目录
  kitty_enc -a FILE [-p] -s DIR     # 任意文件模式[+密码]：使用文件内容[和密码]派生密钥加密指定目录
  kitty_enc -a FILE [-p] -d DIR     # 任意文件模式[+密码]：使用文件内容[和密码]派生密钥解密指定目录

注意事项:
  * 默认是加密模式, 使用-d [<DIR>]来启用解密模式, 未指定目录则为当前目录
  * 加密后会删除原始文件，解密后会删除加密文件
  * 如果使用-r, 程序会递归指定的目录, 请确保指令正确
  * -r可指定递归的深度, 不指定或指定为0代表无限递归
  * 要处理单个文件可以指定文件路径

其他帮助: 
  * 更加详细的说明请使用help或--help指令查看
  * 子命令帮助请在子命令下查看
...
注意事项:
  ...
  - 任意文件模式 (-a):
      * 使用任意文件的内容派生密钥
      * 文件大小不得小于默认密钥文件的最小值
      * 程序会读取文件最多前1MB内容用于密钥派生
      * 可以结合密码使用
      * 适用于需要基于特定文件生成密钥的场景
  - 修复模式 (-f):
      * 希望永远不要用到它, 应当优先手动进行修复
